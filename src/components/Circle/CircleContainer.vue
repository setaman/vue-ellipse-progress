...
import GradientColor from "../Gradients/GradientColor.vue";
import HalfCircleProgress from "./HalfCircle.vue";
import CircleProgress from "./CircleProgress.vue";
import { computed } from "vue";
import ColorDefinitions from "@/components/Gradients/ColorDefinitions.vue";
import { type CircleProps, DEFAULT_SIZE } from "@/utils.ts";

const props = defineProps<CircleProps>();

const circleType = computed(() => {
  return props.half ? HalfCircleProgress : CircleProgress;
});
const size = computed(() => {
...
<template>
  <div class="ep-svg-container" :class="{ 'ep-reverse': props.reverse }">
    <svg class="ep-svg" :height="size" :width="size" xmlns="http://www.w3.org/2000/svg">
      <g class="ep-circle--container">
        <color-definitions v-bind="props" />
        <component :is="circleType" v-bind="props">
    ...
    <!--    <circle-dot v-if="dot.size" :options="parsedProps" />-->
  </div>
</template>
<style lang="scss">
.ep-svg-container {
